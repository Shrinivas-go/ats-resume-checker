...
import { apiPath, getApi404Message } from '../config/api';
    ...
    const handleParseResume = async () => {
        ...
        try {
            ...
            if (err.response?.status === 404) {
                setError(getApi404Message(err.response?.data));
            } else if (err.response?.status === 422) {
                setError(err.response?.data?.message || 'Could not read this PDF. Try exporting it again as PDF.');
            } else {
                const msg = err.response?.data?.message || err.message || 'Unable to parse resume';
                setError(msg.startsWith('Server error') ? msg : 'Server error: ' + msg);
            }
        } finally {
