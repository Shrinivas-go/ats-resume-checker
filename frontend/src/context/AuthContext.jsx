...
export function AuthProvider({ children }) {
    ...
    const googleLogin = async (credential) => {
        try {
            ...
            let message = err.response?.data?.message || 'Google login failed';

            if (!err.response) {
                message = `Cannot reach API at ${API_URL}. Check VITE_API_URL on Vercel and CORS (FRONTEND_URL) on Render.`;
            } else if (err.response.status === 503) {
                message = 'Server database is unavailable. Try again in a moment.';
            }
