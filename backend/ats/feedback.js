...
function generateRecommendations(
  ...
  if (missingCoreSkills.length > 0) {
    if (missingCoreSkills.length === 1) {
      recommendations.push(`Add "${missingCoreSkills[0]}" to your resume - this is a critical requirement for the role.`);
    } else if (missingCoreSkills.length <= 3) {
      recommendations.push(`Include these essential skills in your resume: ${missingCoreSkills.join(', ')}. These are core requirements that hiring managers look for.`);
    } else {
      const topMissing = missingCoreSkills.slice(0, 3);
      recommendations.push(`Focus on adding these critical skills first: ${topMissing.join(', ')}. You're also missing ${missingCoreSkills.length - 3} other core requirements.`);
    }
  }

  if (atsScore < 40 && missingCoreSkills.length > 3) {
    recommendations.push(`Consider gaining experience in the core areas where you're lacking qualifications before applying. This will significantly improve your chances.`);
  }

  if (matchedCoreSkills.length > 0 && atsScore < 70) {
    recommendations.push(`Make sure your existing skills (${matchedCoreSkills.slice(0, 3).join(', ')}) are prominently featured in your resume summary and work experience.`);
  }

  if (missingOptionalSkills.length > 0 && missingCoreSkills.length < 3) {
    if (missingOptionalSkills.length <= 2) {
      recommendations.push(`To stand out from other candidates, consider adding: ${missingOptionalSkills.join(', ')}.`);
    } else {
      const topOptional = missingOptionalSkills.slice(0, 2);
      recommendations.push(`To strengthen your application further, consider highlighting these additional skills if you have them: ${topOptional.join(', ')}.`);
    }
  }

  if (atsScore >= 60 && atsScore < 80) {
    recommendations.push(`Use industry-standard terms and keywords naturally throughout your resume to improve visibility in applicant tracking systems.`);
  }

  if (atsScore >= 80) {
    recommendations.push(`Your resume is well-aligned with the job requirements. Ensure your experience section provides specific examples of how you've applied these skills.`);
  }

  if (recommendations.length === 0) {
    recommendations.push(`Review the job description carefully and tailor your resume to emphasize relevant experience and accomplishments.`);
  }
